...
Network = function() {
  ...
  update = function() {
    ...
    if (layout === "radial") {
      __users = sorted__users(curNodesData, curLinksData);
      updateCenters(__users);
    }
  ...
  network.updateSearch = function(searchTerm) {
    ...
    return node.each(function(d) {
      ...
      match = d._name.toLowerCase().search(searchRegEx);
  ...
  setupData = function(data) {
    ...
    data.nodes.forEach(function(n) {
      ...
      return n.radius = circleRadius(n.reviewcount);
    });
  ...
  filterNodes = function(allNodes) {
    var cutoff, filteredNodes, reviewcounts;
    filteredNodes = allNodes;
    if (filter === "popular" || filter === "obscure") {
      reviewcounts = allNodes.map(function(d) {
        return d.reviewcount;
      }).sort(d3.ascending);
      cutoff = d3.quantile(reviewcounts, 0.5);
      filteredNodes = allNodes.filter(function(n) {
        if (filter === "popular") {
          return n.reviewcount > cutoff;
  ...
  sorted__users = function(nodes, links) {
    var counts, __users;
    __users = [];
    if (sort === "links") {
      counts = {};
      links.forEach(function(l) {
        var _name, _name1;
        if (counts[_name = l.source._name] == null) {
          counts[_name] = 0;
        }
        counts[l.source._name] += 1;
        if (counts[_name1 = l.target._name] == null) {
          counts[_name1] = 0;
        }
        return counts[l.target._name] += 1;
      });
      nodes.forEach(function(n) {
        var _name;
        return counts[_name = n._name] != null ? counts[_name] : counts[_name] = 0;
      });
      __users = d3.entries(counts).sort(function(a, b) {
        return b.value - a.value;
      });
      __users = __users.map(function(v) {
        return v.key;
      });
    } else {
      counts = nodeCounts(nodes, "__users");
      __users = d3.entries(counts).sort(function(a, b) {
        return b.value - a.value;
      });
      __users = __users.map(function(v) {
  ...
  updateCenters = function(__users) {
    if (layout === "radial") {
      return groupCenters = RadialPlacement().center({
        "x": width / 2,
        "y": height / 2 - 100
      }).radius(300).increment(18).keys(__users);
    }
  };
  ...
  moveToRadialLayout = function(alpha) {
    ...
    return function(d) {
      var centerNode;
      centerNode = groupCenters(d._name);
      d.x += (centerNode.x - d.x) * k;
      return d.y += (centerNode.y - d.y) * k;
    };
  };
  strokeFor = function(d) {
    return d3.rgb(nodeColors(d._name)).darker().toString();
  };
  showDetails = function(d, i) {
    var content;
    content = '<p class="main">' + d._name + '</span></p>';
...
$(function() {
  ...
  $("#data_select").on("change", function(e) {
    var dataFile;
    dataFile = $(this).val();
    return d3.json("data/" + File, function(json) {
  ...
  return d3.json("data/test.json", function(json) {
